classdef Analyzer < handle
    ...
    properties (Access = private)
        ...
        piv_processor__data Array_temp_storage
    ...
    methods
        function obj = Analyzer( ap, rp, pp, pf, pg, piv_processor__data, fi_processor__data)
            obj.ap = ap;
            obj.rp = rp;
            obj.pp = pp;
            obj.pf = pf;
            obj.pg = pg;
            obj.piv_processor__data = piv_processor__data;
        ...
        function [ts, pos_ij_arr] = Get_time_series_advanced(obj, pos_ij_arr)
            %Get_time_series_advanced расширенная версия Get_time_series
            %             if pos_i > size(pg.xMat,1) || pos_j > size(pg.xMat,2), return; end
            if isempty(pos_ij_arr) %выбираем все
                [pos_i, pos_j] = meshgrid( 1:size(obj.pg.xMat,1), 1:size(obj.pg.xMat,2) );
                pos_ij_arr = [ pos_i(:), pos_j(:) ];
            ...
            ts = nan( obj.pf.frame_count, 9, size(pos_ij_arr,1) );
            
            for ti = 1:obj.pf.frame_count
                disp([ti obj.pf.frame_count]);
                t = ( obj.pf.first_frames(ti) - 1 ) / obj.rp.fps;
                d_piv =  obj.piv_processor__data.Get(ti);
                d_fi  =  obj.fi_processor__data. Get(ti);
